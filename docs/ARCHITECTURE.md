...
Generated once, at installation, by `atrium-core init-identity` running as
the `atrium` user before the installer locks `/etc/atrium` down — never by the
running service, which can read its identity and cannot replace it:

- `server_id` — 128 random bits, stable for the life of the installation, stored
  in `/etc/atrium/identity.json` (`root:atrium 0640` after installation).
- a TLS key pair (`/etc/atrium/tls.key`, same ownership) and a self-signed leaf
  certificate in the state directory. SANs include the current addresses,
  `atrium-<short-id>.local`, and `localhost`. The certificate is reissued
  automatically when addresses change; **the key pair is not**.

A missing or inconsistent identity puts Core in recovery mode; nothing
regenerates it. As built in M1B — see the plan's M1B *As built* note.
...
| --- | --- | --- | --- |
| Identity, TLS key | `/etc/atrium/` | **root**, read-only to Core | `root:atrium 0640` in a `root:atrium 0750` directory: the `atrium` group may traverse and read, and has no write bit, so Core can read the key to serve TLS and cannot create, unlink, rename over or chmod it; generation is a one-off install-time step and rotation is a console action. The public certificate lives in the writable state directory. |
| State: devices, users, apps, settings, audit, operations | `/var/lib/atrium/atrium.db` | Core | SQLite, WAL |
| Secrets (app credentials, tokens) | `/var/lib/atrium/secrets.db` (from M3) | Core | SQLite, encrypted with `/etc/atrium/secrets.key` — `root:atrium 0640`, generated at installation in M1, first used in M3 |
